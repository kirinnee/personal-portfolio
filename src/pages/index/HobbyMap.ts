...
const hobbyMap = [
	...
	new HobbyData("Anime Capital", images.sideProject.anime,
		"<p>I am a fan of Japanese anime culture. However, it is difficult to procure or buy disks or services to view the seasonal series that are currently airing, much less subbed series. As a little side-project for my fellow anime friends and myself, I developed Anime Capital, which is a website that streams the newest anime scrapped from various sites. </p>"
		+ "<p>Anime Capital uses a Ruby-based program to scrape the web for English-subbed anime and encode the subtitle into an MP4 file with multiple bitrates. After this, it splits the file into 3-second binaries containing a .mpd manifest that is pushed to a .NET core server while the binaries are pushed to a bucket. The frontend website, running on Vue.js, would then query the API server for the links to these binaries, automatically allowing streaming and continuous updating of anime. This project is fully open-sourced at GitLab.</p>"
		,
		"https://anime.capital", [
			images.black.rb,
			images.icons.frontend.vue,
